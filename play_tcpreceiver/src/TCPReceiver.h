...
private:
  ...
  int btnCheck()
  {
    ...
    //if(nowtime - ltime < 10 && nowtime > ltime){return 0;}
    for( int i = 0; i < 3; i++){
      b = 'A' + i;
      pinVal = digitalRead(BUTTON_A_PIN - i); //A:GPIO39,B:GPIO38,C:GPIO37
      //Active Low
      if(pinVal){
        if(st_btn[i].sts > 0 && nowtime-st_btn[i].oms >= 100){
            if( --st_btn[i].sts <= 0 ){
              s = 'F';  //Release
              break;
            }
      ...
      //HIGH
        ...
        //押しっぱなしは200msecごとにオンを送信する
        if( st_btn[i].sts >= 2 && (nowtime - st_btn[i].oms >= 500 || nowtime < st_btn[i].oms) ){
            s = 'T';
            st_btn[i].oms = nowtime;
            break;
        }
      }
    }
